...
def resolve_vss_url(body: VSSResolveRequest):
    """Gộp toàn bộ flow VSS: login → query device → build FLV URL.

    Nhận RealVideo URL, trả về FLV stream URL sẵn sàng dùng cho FFmpeg.
    """
    url = body.url.strip()
...
def vss_validate_url(url: str = Query(..., description="URL cần validate")):
    ...
    return VSSValidateResponse(
        ...
        uses_ffmpeg_backend=uses_vss_backend(url),
        device_id=device_id,
        channel=channel,
        stream=stream_val,
